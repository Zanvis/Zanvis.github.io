...
export class CoreComponent {
  projects: Project[] = [
    ...
    { title: 'Transform App', category: 'C#', date: new Date('2024-03-17'), image: 'CSharpImages/Transform/thumb.png', videoSrc: 'CSharpImages/Transform/video.mp4', type: 'School project', link: 'https://github.com/Zanvis/Transform-App', description: 'This is a school project written in C#. The application is designed to be a geometric transformation tool that allows users to create and manipulate polygons. Users can select the number of polygon sides, and the program generates the corresponding shape. The interface provides options for various transformations, including translation, rotation, rotation around a specific point, scaling, reflection (both X and Y axes), and shearing (both X and Y directions). The coordinates of the polygon vertices are displayed in a table on the left, and there are buttons to start the transformation and reset the shape.' },
